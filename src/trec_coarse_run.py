...
def load_data() -> SplitedData:
    ...
    train_y = list(map(lambda x: LABEL_MAP[x], train_dataset['coarse_label']))
    valid_x = valid_dataset['text']
    valid_y = list(map(lambda x: LABEL_MAP[x], valid_dataset['coarse_label']))
    test_x = test_dataset['text']
    test_y = list(map(lambda x: LABEL_MAP[x], test_dataset['coarse_label']))
